...
def load_html_search():
    ...
    url = 'https://listado.mercadolibre.com.mx/{}#D[A:{}]'.format(search.replace(' ','-'), search.replace(' ','%20'))
    ...
    if pages_flag.__len__() != 0:
        ...
        for i in range(df.__len__()):
            ...
            score_rating_tag_filter = soup_.find('div', {"class":"ui-review-capability"})
            
            if score_rating_tag_filter is None:
                scores.append(0.0)
                ratings.append(0)
            else:
                while soup_.find('div', {"class":"ui-review-capability"}) is None:
                    link_prod = df.loc[i, 'Link']
                    request_ = requests.get(link_prod)
                    content_ = request_.text
                    soup_ = bs4.BeautifulSoup(content_, 'html.parser') # HTML document
                ...
                ratings.append(int(re.match(r'\d+',soup_.find('div', {"class":"ui-review-capability"}).find(class_="ui-review-capability__rating__label").text)[0]))

            comments_tag_filter = soup_.find_all('article', {"class":"ui-review-capability-comments__comment"})

            comments_prod = []
            if (comments_tag_filter is None) | (comments_tag_filter.__len__() == 0):
                comments_prod.append([])
            else:
                while soup_.find_all('article', {"class":"ui-review-capability-comments__comment"}).__len__() == 0:
                    link_prod = df.loc[i, 'Link']
                    request_ = requests.get(link_prod)
                    content_ = request_.text
                    soup_ = bs4.BeautifulSoup(content_, 'html.parser') # HTML document
        ...
        df = df[['Brand','Product', 'Price','Score','Ratings','Comments','Link']]
